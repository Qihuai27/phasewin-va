...
from attribution_research.visualization import render_classification_report


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Render classification attribution reports.")
    parser.add_argument("--model", required=True, help="Classification model key.")
...
def main() -> None:
    ...
    preset = select_presets("classification", args.model)[0]
    ...
    if not method_dirs:
        raise FileNotFoundError(f"No completed classification result dirs found under {result_root}")
    ...
    print(f"Saved classification visualizations to {output_root}")
